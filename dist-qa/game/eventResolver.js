...
function applyDelta(state, delta, summary) {
    ...
    if (delta.heat) {
        const heatMult = (0, worldEvents_1.getCombinedHeatMultiplier)(base, (0, locations_1.getPlayerAreaKey)(base.player));
    ...
    if (delta.applyPriceMultiplier) {
        const loc = (0, locations_1.getPlayerAreaKey)(player);
    ...
    player = (0, engineHelpers_1.checkGameOverState)({ ...base, player }).player;
...
const CHOICE_RESOLVERS = {
    ...
    spike_sell: (state, event) => {
        ...
        const price = state.marketPrices[(0, locations_1.getPlayerAreaKey)(state.player)]?.[commodityId] ?? 0;
        if (price <= 0) {
    ...
    informant_pay: (state, event) => {
        ...
        const loc = event.context.locationId ?? (0, locations_1.getPlayerAreaKey)(state.player);
        const parsed = (0, locations_1.parseAreaKey)(loc);
        const locName = parsed
            ? (0, locations_1.getAreaLabel)(parsed.cityId, parsed.areaId)
            : loc;
