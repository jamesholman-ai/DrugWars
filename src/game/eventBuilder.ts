...
import { EventContext, EventType, GameEvent } from '../types/events';
import { COMMODITIES } from '../data/commodities';
import { AREAS, CITIES, getAreaKey, getAreasForCity } from '../data/locations';
import { randomInt, pickRandom } from '../utils/random';
import { buildNpcBlock } from './eventBuilderHelpers';
...
function ctx(partial: EventContext): EventContext {
  return partial;
}

const BUILDERS: Record<EventType, EventBuilder> = {
  police_stop: (state, random) => {
    const bribe = randomInt(400, 700);
    const npc = buildNpcBlock(state, 'vance', random);
    return {
  ...
  police_raid: (state, random) => ({
    ...
    context: ctx({ quantity: randomInt(3, 10), amount: randomInt(250, 450) }),
    choices: [
  ...
  rival_dealer: (state, random) => {
    const tribute = randomInt(300, 500);
    const npc = buildNpcBlock(state, 'razor', random);
    return {
  ...
  robbery_attempt: (state, random) => {
    const demand = randomInt(300, 600);
    return {
  ...
  supplier_discount: (state, random) => {
    ...
    const qty = randomInt(6, 12);
    const cost = randomInt(400, 900);
    const npc = buildNpcBlock(state, 'mama_silk', random);
    return {
  ...
  price_spike: (state, random) => {
    ...
    return {
      ...
      context: ctx({ commodityId: commodity.id, priceMultiplier: mult, quantity: randomInt(3, 8) }),
      choices: [
  ...
  price_crash: (state, random) => {
    ...
    return {
      ...
      context: ctx({ commodityId: commodity.id, priceMultiplier: mult, amount: cost, quantity: randomInt(4, 8) }),
      choices: [
  ...
  informant_tip: (state, random) => {
    ...
    const cityAreas = getAreasForCity(city.id);
    const area = cityAreas.length ? pickRandom(cityAreas) : pickRandom(AREAS);
    const locationKey = getAreaKey(city.id, area.id);
    const cost = randomInt(200, 400);
    const npc = buildNpcBlock(state, 'whisper', random);
    return {
      id: `evt_${state.player.day}_informant`,
      eventType: 'informant_tip',
      title: 'Informant Tip',
      description: `${npc.name} slides up. "Got a line on cheap ${commodity.name} near ${city.name} ${area.name}. $${cost} for the details."`,
      npc,
      context: ctx({ commodityId: commodity.id, locationId: locationKey, amount: cost, npcId: 'whisper' }),
      choices: [
  ...
  bulk_buyer_offer: (state, random) => {
    const commodity = pickRandom(COMMODITIES);
    const qty = randomInt(8, 15);
    const premium = randomInt(900, 1800);
    const npc = buildNpcBlock(state, 'chip', random);
    return {
  ...
  health_emergency: (state, random) => {
    const clinic = randomInt(300, 500);
    const favor = randomInt(600, 1200);
    return {
  ...
  debt_collector_warning: (state, random) => {
    const partial = randomInt(400, 800);
    const penalty = randomInt(300, 600);
    const npc = buildNpcBlock(state, 'bruno', random);
    return {
...
export function getAllEventTypes(): EventType[] {
  return Object.keys(BUILDERS) as EventType[];
}
